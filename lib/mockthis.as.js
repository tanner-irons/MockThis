...
import { makeUnflat, getRandomArrayLength } from './mockthis.utilities';
...
let _generateObject = function (blueprint) {
    ...
    blueprint.sortedSchema.forEach((prop) => {
        ...
        if ((/.0/g).test(prop.property)) {
            let arrayLength = getRandomArrayLength(blueprint.array.min, blueprint.array.max);
...
let _generateData = function (blueprint) {
    ...
    let arrayLength = getRandomArrayLength(blueprint.total.min, blueprint.total.max);
    for (let i = 0; i < arrayLength; i++) {
